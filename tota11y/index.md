...
<p>Some silly sausage (me) forgot that the marvellous HTML5 *&lt;details&gt; element is keyboard-focussable, so that's now also in the Focus Order plugin. As it can contain other focussable elements, the plugin will open all details blocks so that tab order can be displayed (and give the auto-opened details element a green outline).</p>
